    ...
    setModmail() {
        this.client.on("messageCreate", message => {
            if (message.channel.type === discord_js_1.ChannelType.DM && !message.author.bot) {
                ...
                if (!guild?.members.cache.some(x => x.id === message.author.id))
                    return;
        ...
        this.client.on("interactionCreate", async (interaction) => {
            if (interaction.isChatInputCommand() && !interaction.user.bot && interaction.guildId == this.config.guildId) {
                const isTicketChannel = interaction.channel?.type == discord_js_1.ChannelType.GuildText && interaction.channel.parentId == interaction.guild?.channels.cache.find(x => x.id == this.config.categoryId)?.id, categoryName = this.client.guilds.cache.get(this.config.guildId)?.channels.cache.get(this.config.categoryId)?.name;
