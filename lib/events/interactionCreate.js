...
const event = async ({ config, client }, interaction) => {
    if (interaction.isChatInputCommand() && !interaction.user.bot && interaction.guildId == config.guildId) {
        const isTicketChannel = interaction.channel?.type == discord_js_1.ChannelType.GuildText && interaction.channel.parentId == interaction.guild?.channels.cache.get(config.categoryId)?.id, categoryName = client.guilds.cache.get(config.guildId)?.channels.cache.get(config.categoryId)?.name;
